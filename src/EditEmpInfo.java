...
public class EditEmpInfo extends javax.swing.JFrame {
     ...
     */
   public EditEmpInfo(int empNum, boolean readOnly, User user) {
       initComponents(); // sets up the screen

       this.empNumToEdit = empNum;
       this.readOnly = readOnly;
       this.loggedInUser = user; // store who's logged in

       loadEmployeeData(empNumToEdit); // show employee info

       FirstName = new JTextField();  // not important for this fix

       if (readOnly && !isHRorIT()) {
           disableEditing(); // only lock buttons for Employee or Supervisor
       }

       // new line that hides Edit and Save if needed
       checkAccessAndHideButtons();

       // keeps formatting for IDs
   ...
   private boolean isHRorIT() {
    if (loggedInUser == null) return false;
    String role = loggedInUser.getAccess().toLowerCase();
    return role.contains("hr") || role.contains("it");
}
   
   private void checkAccessAndHideButtons() {
   ...
   private void saveEmployeeUpdates() {
    ...
    private void updateLoginCSV() {
        String filePath = "src/data/MotorPHlogin.csv";  // adjust path if needed
        List<String[]> allRows = EmployeeFileHandler.readRawCSV(filePath);

        for (String[] row : allRows) {
            // Skip header
            if (row[0].equalsIgnoreCase("Employee Number")) continue;

            if (row[0].trim().equals(String.valueOf(employeeData.getEmployeeNumber()))) {
                System.out.println("MATCH FOUND: Updating MotorPHlogin row for EmpNum " + row[0]);
                row[1] = employeeData.getLastName();
                row[2] = employeeData.getFirstName();
                row[4] = employeeData.getPosition();
                row[5] = employeeData.getSupervisor();
                break;
            }
        }

        EmployeeFileHandler.writeRawCSV(filePath, allRows);
    }
